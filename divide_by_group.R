...
# loading in metadata
meta <- read.xlsx(file = args[1], sheetIndex = 1)
...
csv_files <- list.files(path = args[2], pattern = "\\.csv$")
...
for (i in csv_files) {
  ...
  if (gsub("[^0-9]", "", i) %in% flr$sample) {
    ...
    count_matrix <- read.csv(file = paste(args[2], "/", i, sep = ""))
...
# jpeg(paste(args[2], "/", "flr_stacked.jpg", sep = ""))
# 
...
# dev.off()
# 
# jpeg(paste(args[2], "/", "flr_percent_stacked.jpg", sep = ""))
# 
# ggplot(flr_stack, aes(fill = condition, y = value, x = sample)) + 
...
# jpeg(paste(args[2], "/", "nonflr_stacked.jpg", sep = ""))
# 
...
# dev.off()
# 
# jpeg(paste(args[2], "/", "nonflr_percent_stacked.jpg", sep = ""))
# 
# ggplot(nonflr_stack, aes(fill = condition, y = value, x = sample)) + 
#   geom_bar(position = "fill", stat = "identity") + 
#   xlab("Sample") + ylab("percentage") + 
...
# jpeg(paste(args[2], "/", "ctrlwp_stacked.jpg", sep = ""))
# 
...
# dev.off()
# 
# jpeg(paste(args[2], "/", "ctrlwp_percent_stacked.jpg", sep = ""))
# 
# ggplot(ctrlwp_stack, aes(fill = condition, y = value, x = sample)) + 
#   geom_bar(position = "fill", stat = "identity") + 
#   xlab("Sample") + ylab("percentage") + 
...
# jpeg(paste(args[2], "/", "ctrlwop_stacked.jpg", sep = ""))
# 
...
# dev.off()
# 
# jpeg(paste(args[2], "/", "ctrlwop_percent_stacked.jpg", sep = ""))
# 
# ggplot(ctrlwop_stack, aes(fill = condition, y = value, x = sample)) + 
#   geom_bar(position = "fill", stat = "identity") + 
#   xlab("Sample") + ylab("percentage") + 
