...
# helper function for loading count matrices into data frames
load_counts <- function(stack, dir, fname, e_name, i_name) {
  
  count_m <- read.csv(file = paste(dir, "/", fname, sep = ""))
  
  temp <- data.frame(sample = rep(gsub("[^0-9]", "", i), 2),
                     condition = c("exons", "introns"),
                     value = c(sum(count_m[,e_name]), sum(count_m[,i_name])))
...
for (i in csv_files) {
  ...
  if (gsub("[^0-9]", "", i) %in% flr$sample) {
    ...
    # load_counts(flr_stack, csv_dir, i, "exon_count", "intron_count")
    ...
    # load_counts(nonflr_stack, csv_dir, i, "exon_count", "intron_count")
    count_m <- read.csv(file = paste(csv_dir, "/", i, sep = ""))
    ...
    # load_counts(ctrlwp_stack, csv_dir, i, "exon_count", "intron_count")
    count_m <- read.csv(file = paste(csv_dir, "/", i, sep = ""))
    ...
    # load_counts(ctrlwop_stack, csv_dir, i, "exon_count", "intron_count")
    count_m <- read.csv(file = paste(csv_dir, "/", i, sep = ""))
